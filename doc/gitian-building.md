...
*Setup instructions for a gitian build of News24coin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the News24coin
...
- [Building News24coin](#building-news24coin)
...
Clone the git repositories for news24coin and gitian and then checkout the news24coin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/news24coin-crypto/news24coin.git
cd news24coin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the news24coin repository
...
To build News24coin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the news24coin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/news24coin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/news24coin-crypto/news24coin
...
For example:
```bash
URL=https://github.com/crowning-/news24coin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit news24coin=${COMMIT} --url news24coin=${URL} ../News24coin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit news24coin=${COMMIT} --url news24coin=${URL} ../News24coin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit news24coin=${COMMIT} --url news24coin=${URL} ../News24coin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/news24coin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/news24coin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/news24coin-build.assert
```
...
[news24coin/gitian.sigs](https://github.com/news24coin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
